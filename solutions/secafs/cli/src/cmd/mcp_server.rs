use secafs_sdk::{SecAFS, SecAFSOptions, Stats};
...
pub async fn handle_mcp_server_command(
    ...
    // Resolve and open SecAFS
    let options = SecAFSOptions::resolve(&id_or_path).context(format!(
    ...
    let server = McpServer::new(secafs_inst, tools_filter);
...
impl McpServer {
    fn new(secafs: SecAFS, tools_filter: Option<Vec<String>>) -> Self {
...
impl McpServer {
    ...
    async fn handle_write_file(&self, params: WriteFileParams) -> Result<String> {
        ...
        if self.secafs.fs.stat(&path).await?.is_some() {
            self.secafs.fs.remove(&path).await?;
        }
        let (_, file) = self
            .secafs
            .fs
    ...
    fn ensure_parent_dirs<'a>(
        ...
        Box::pin(async move {
            ...
            if self.secafs.fs.stat(&parent_path).await?.is_some() {
                return Ok(());
            }
    ...
    async fn handle_access(&self, params: AccessParams) -> Result<String> {
        ...
        let exists = self.secafs.fs.stat(&path).await?.is_some();
    ...
    fn collect_file_resources<'a>(
        ...
        Box::pin(async move {
            let dir_stats = match self.secafs.fs.stat(path).await? {
                Some(s) => s,
                None => return Ok(()),
            };

            let entries = match self.secafs.fs.readdir(dir_stats.ino).await? {
            ...
            for entry in entries {
                ...
                let stats = match self.secafs.fs.stat(&full_path).await? {
                    Some(s) => s,
                    None => continue,
                };
