...
impl Shell {
    ...
    fn completion_line(&self) -> &'static str {
        match self {
            Shell::Bash => "source <(COMPLETE=bash secafs)",
            Shell::Zsh => "source <(COMPLETE=zsh secafs)",
            Shell::Fish => "COMPLETE=fish secafs | source",
            Shell::Elvish => "eval (COMPLETE=elvish secafs | slurp)",
            Shell::PowerShell => "$env:COMPLETE = \"powershell\"; secafs | Out-String | Invoke-Expression; Remove-Item Env:\\COMPLETE",
        }
    }
}
