#!/bin/sh
#
# Test syscalls through secafs run --experimental-sandbox (ptrace-based).
#
# This tests SecAFS file operations using the ptrace-based sandbox
# where files are stored in the SecAFS database at /agent.
#
set -e

echo -n "TEST syscalls (secafs run --experimental-sandbox)... "
