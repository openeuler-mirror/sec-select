#!/bin/sh
#
# Test syscalls through secafs run (FUSE overlay).
#
...
echo -n "TEST syscalls (secafs run - FUSE overlay)... "
...
# - Modifications go to the delta layer (SecAFS database)
...
# verify it with a separate secafs run. The "All tests passed!" check is sufficient.
