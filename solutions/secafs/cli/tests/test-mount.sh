...
MOUNTPOINT="/tmp/secafs-test-mount-$$"
...
cargo run -- mount ".secafs/${TEST_AGENT_ID}.db" "$MOUNTPOINT" --foreground &
MOUNT_PID=$!
...
# Test that 'secafs mount' (no args) lists our mount
if ! cargo run -- mount 2>/dev/null | grep -q "$MOUNTPOINT"; then
    echo "FAILED: 'secafs mount' did not list our mountpoint"
    kill $MOUNT_PID 2>/dev/null || true
    exit 1
fi
