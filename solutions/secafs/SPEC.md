...
The Agent Filesystem Specification defines a relational schema for representing agent filesystem state, stored in a PostgreSQL/openGauss database. The specification consists of three main components:
...
- `status` - Call status: `'pending'`, `'success'`, or `'error'`
- `started_at` - Invocation timestamp (Unix timestamp, seconds)
- `completed_at` - Completion timestamp (Unix timestamp, seconds; NULL while pending)
- `duration_ms` - Execution duration in milliseconds (NULL while pending)
...
- `owner` - Database role that created the inode (NULL for shared/unowned), used for per-role access control
...
- `data` - Binary content (BYTEA), exactly `chunk_size` bytes except for the last chunk
...
- Lookups are keyed on the `path` primary key
- Child whiteouts within a directory are found by matching the directory prefix against `path`
...
```sql
INSERT INTO fs_origin (delta_ino, base_ino)
VALUES (?, ?)
...
3. The `path` MUST be a normalized absolute path
