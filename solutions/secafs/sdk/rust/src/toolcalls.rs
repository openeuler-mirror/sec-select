...
/// Tool calls tracker backed by the SecAFS database
#[derive(Clone)]
...
impl ToolCalls {
    ...
    fn row_to_tool_call(row: &DbRow) -> Result<ToolCall> {
        let id = row
            .get_value(0)
            .ok()
            .and_then(|v| v.as_integer().copied())
    ...
    fn row_to_stats(row: &DbRow) -> Result<ToolCallStats> {
        let name = row
            .get_value(0)
            .ok()
            .and_then(|v| match v {
