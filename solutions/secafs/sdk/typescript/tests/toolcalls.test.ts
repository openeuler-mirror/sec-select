...
describe("ToolCalls Integration Tests", () => {
  ...
  describe("Basic Operations", () => {
    ...
    it("should mark a tool call as successful", async () => {
      ...
      expect(Number(toolCall?.completed_at)).toBeGreaterThan(0);
      expect(Number(toolCall?.duration_ms)).toBeGreaterThanOrEqual(0);
    });
    ...
    it("should get a tool call by ID", async () => {
      ...
      expect(toolCall).toBeDefined();
      expect(Number(toolCall?.id)).toBe(id);
      expect(toolCall?.name).toBe("test_tool");
      expect(toolCall?.parameters).toEqual({ arg: "value" });
      expect(toolCall?.status).toBe("pending");
      expect(Number(toolCall?.started_at)).toBeGreaterThan(0);
    });
  ...
  describe("Query Operations", () => {
    ...
    it("should get recent tool calls", async () => {
      ...
      expect(results.every((tc) => Number(tc.started_at) >= midpoint)).toBe(true);
    });
  ...
  describe("Statistics", () => {
    it("should calculate tool call statistics", async () => {
      ...
      expect(Number(toolAStats?.total_calls)).toBe(3);
      expect(Number(toolAStats?.successful)).toBe(2);
      expect(Number(toolAStats?.failed)).toBe(1);
      expect(Number(toolAStats?.avg_duration_ms)).toBeGreaterThanOrEqual(0);

      const toolBStats = stats.find((s) => s.name === "tool_b");
      expect(toolBStats).toBeDefined();
      expect(Number(toolBStats?.total_calls)).toBe(1);
      expect(Number(toolBStats?.successful)).toBe(1);
      expect(Number(toolBStats?.failed)).toBe(0);
    });
    ...
    it("should exclude pending calls from statistics", async () => {
      ...
      expect(toolAStats).toBeDefined();
      expect(Number(toolAStats?.total_calls)).toBe(1); // Only completed calls
    });
  ...
  describe("Ordering", () => {
    it("should return tool calls ordered by started_at desc", async () => {
      ...
      for (let i = 0; i < recent.length - 1; i++) {
        expect(Number(recent[i].started_at)).toBeGreaterThanOrEqual(
          Number(recent[i + 1].started_at)
        );
      }
    });
  });
