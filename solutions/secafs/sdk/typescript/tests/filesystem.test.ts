...
import { SecAFS as Filesystem } from "../src/filesystem/secafs.js";
...
describe("Filesystem Integration Tests", () => {
  ...
  describe("Standalone Usage", () => {
    it("should work with a freshly provisioned database", async () => {
      const standalone = await setupTestDb();
      try {
        const standaloneFs = await Filesystem.fromDatabase(standalone.db);
    ...
    it("should maintain isolation between instances", async () => {
      const a = await setupTestDb();
      const b = await setupTestDb();
      try {
        const fs1 = await Filesystem.fromDatabase(a.db);
        const fs2 = await Filesystem.fromDatabase(b.db);
  ...
  describe("Edge Case Tests", () => {
    ...
    it("should handle empty file with zero chunks", async () => {
      ...
      expect(Number(stats.size)).toBe(0);
    });
    ...
    it("should overwrite large file with smaller file and clean up chunks", async () => {
      ...
      expect(Number(stats.size)).toBe(100);
    });
  ...
  describe("Schema Tests", () => {
    ...
    it("should store chunks with correct ordering in database", async () => {
      ...
      const indices = rows.map((r) => Number(r.chunk_index));
  ...
  describe("Cleanup Tests", () => {
    it("should delete all chunks when file is removed", async () => {
      ...
      expect(Number(beforeResult.count)).toBe(4);
      ...
      expect(Number(afterResult.count)).toBe(0);
    });
    ...
    it("should handle multiple files with different sizes correctly", async () => {
      ...
      for (const [path, size] of files) {
        ...
        expect(Number(countResult.count)).toBe(expectedChunks);
      }
    });
  });
});
