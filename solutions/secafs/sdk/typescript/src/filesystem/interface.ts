...
export function createStats(data: StatsData): Stats {
  // The integer/BIGINT columns these values come from may arrive as strings
  // from the database driver; coerce them to numbers so the Stats fields and
  // the bitmask checks below behave like Node's numeric fs.Stats.
  const mode = Number(data.mode);
  const normalized: StatsData = {
    ino: Number(data.ino),
    mode,
    nlink: Number(data.nlink),
    uid: Number(data.uid),
    gid: Number(data.gid),
    size: Number(data.size),
    atime: Number(data.atime),
    mtime: Number(data.mtime),
    ctime: Number(data.ctime),
  };
  return {
    ...normalized,
 ...
 * allowing implementations like SecAFS (PostgreSQL-backed), HostFS (native filesystem),
