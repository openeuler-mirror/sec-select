 ...
 */
class SecAFSFile implements FileHandle {
  private db: DbClient;
  private bufferCtor: BufferConstructor;
  private ino: number;
  private chunkSize: number;

  constructor(db: DbClient, bufferCtor: BufferConstructor, ino: number, chunkSize: number) {
    this.db = db;
  ...
  async fsync(): Promise<void> {
    // PostgreSQL handles durability via WAL; no explicit fsync needed
  }
...
/**
 * A filesystem backed by PostgreSQL, implementing the FileSystem interface.
 */
export class SecAFS implements FileSystem {
  private db: DbClient;
  private bufferCtor: BufferConstructor;
  private rootIno: number = 1;
  private chunkSize: number = DEFAULT_CHUNK_SIZE;

  private constructor(db: DbClient, b: BufferConstructor) {
    this.db = db;
    this.bufferCtor = b;
  }

  static async fromDatabase(db: DbClient, b?: BufferConstructor): Promise<SecAFS> {
    const fs = new SecAFS(db, b ?? Buffer);
    await fs.initialize();
    return fs;
  }
  ...
  private async getLinkCount(ino: number): Promise<number> {
    ...
    const result = await stmt.get(ino) as { nlink: number | string } | undefined;
    // The nlink column is BIGINT; the pg driver returns it as a string, so
    // coerce to Number before callers compare it against 0.
    return Number(result?.nlink ?? 0);
  }
  ...
  async open(path: string): Promise<FileHandle> {
    ...
    return new SecAFSFile(this.db, this.bufferCtor, ino, this.chunkSize);
  }
