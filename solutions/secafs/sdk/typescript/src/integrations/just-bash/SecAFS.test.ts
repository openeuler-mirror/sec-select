import { SecAFS } from "../../index_node.js";
import { createPostgresDb, type PgDb } from "../../db_node.js";
import { afterEach, beforeAll, beforeEach, describe, expect, it } from "vitest";
import { SecAFSWrapper } from "./SecAFS.js";
...
// Probe the dev database once. When it is unreachable, every test skips
// cleanly instead of producing connection failures.
...
describe("SecAFSWrapper", () => {
  let connected = false;
  let db: PgDb | undefined;
  let schema: string;
  let secafsHandle: Awaited<ReturnType<typeof SecAFS.openWith>>;
  let fs: SecAFSWrapper;

  beforeAll(async () => {
    connected = await canConnect();
  });

  // Each test runs in its own isolated schema so the SDK's `CREATE TABLE IF NOT
  // EXISTS` statements land in a clean namespace, independent of any objects
  // left in the database's default schema.
  beforeEach(async (ctx) => {
    ...
    schema = uniqueSchema();
    // Pool size 1 keeps the per-test `search_path` consistent across queries.
    db = await createPostgresDb(testUrl(), 1);
    await db.exec(`CREATE SCHEMA "${schema}"`);
    await db.exec(`SET search_path TO "${schema}"`);
    secafsHandle = await SecAFS.openWith(db);
    fs = new SecAFSWrapper({ fs: secafsHandle });
  });
  ...
  describe("constructor", () => {
    ...
    it("should accept custom mount point", () => {
      const customFs = new SecAFSWrapper({
        fs: secafsHandle,
        mountPoint: "/mnt/data",
      });
    ...
    it("should throw for non-absolute mount point", () => {
      expect(
        () => new SecAFSWrapper({ fs: secafsHandle, mountPoint: "relative" }),
  ...
  describe("link", () => {
    ...
    it("should create independent copy", async () => {
      ...
      // Since SecAFS uses copyFile, the "hard link" is actually a copy
