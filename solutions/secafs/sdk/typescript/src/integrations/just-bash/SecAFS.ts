/**
 * SecAFS - Read-write filesystem backed by any SecAFS FileSystem implementation
 *
 * Full read-write filesystem with persistent, auditable file storage for AI agents.
 *
 * Supports any FileSystem implementation:
 * - SecAFS (PostgreSQL/openGauss) for Node.js and browser
 * - Cloudflare SecAFS (Durable Objects) for Cloudflare Workers
 */

import type { FileSystem } from "../../filesystem/interface.js";
import { SecAFS, type SecAFSOptions } from "../../index_node.js";
import type {
  BufferEncoding,
  CpOptions,
  FsStat,
 ...
 * Can be from SecAFS.open() (PostgreSQL/openGauss) or SecAFS.create() (Cloudflare).
 */
export interface SecAFSHandle {
  fs: FileSystem;
}

export interface SecAFSJustBashOptions {
  /**
   * The SecAFS handle containing a FileSystem implementation.
   */
  fs: SecAFSHandle;
...
/** Default mount point for SecAFS */
const DEFAULT_MOUNT_POINT = "/";

export class SecAFSWrapper implements IFileSystem {
  private readonly secafsFs: FileSystem;
  private readonly mountPoint: string;

  constructor(options: SecAFSJustBashOptions) {
    this.secafsFs = options.fs.fs;
  ...
  /**
   * Convert virtual path to SecAFS path (strip mount point prefix)
   */
  ...
  async readFileBuffer(path: string): Promise<Uint8Array> {
    ...
    try {
      const data = await this.secafsFs.readFile(agentPath);
  ...
  async writeFile(
    ...
    const agentPath = this.toAgentPath(normalized);
    // SecAFS creates parent directories implicitly
    await this.secafsFs.writeFile(agentPath, Buffer.from(buffer));
  }
  ...
  async exists(path: string): Promise<boolean> {
    ...
    const agentPath = this.toAgentPath(normalized);
    try {
      await this.secafsFs.access(agentPath);
      return true;
    } catch {
      return false;
    }
  ...
  async stat(path: string): Promise<FsStat> {
    ...
    try {
      const stats = await this.secafsFs.stat(agentPath);
      return {
        isFile: stats.isFile(),
  ...
  async lstat(path: string): Promise<FsStat> {
    // SecAFS stat doesn't follow symlinks by default
  ...
  async mkdir(path: string, options?: MkdirOptions): Promise<void> {
    ...
    try {
      await this.secafsFs.mkdir(agentPath);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
  ...
  async readdir(path: string): Promise<string[]> {
    ...
    try {
      const entries = await this.secafsFs.readdir(agentPath);
      return entries.sort();
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      if (msg.includes("not found") || msg.includes("ENOENT")) {
  ...
  async rm(path: string, options?: RmOptions): Promise<void> {
    ...
    try {
      await this.secafsFs.rm(agentPath, {
        force: options?.force,
      ...
      if (msg.includes("EISDIR")) {
        ...
        try {
          await this.secafsFs.rmdir(agentPath);
          return;
        } catch (rmdirErr) {
          const rmdirMsg =
  ...
  async cp(src: string, dest: string, options?: CpOptions): Promise<void> {
    ...
    if (srcStat.isFile) {
      ...
      await this.secafsFs.copyFile(srcAgent, destAgent);
  ...
  async mv(src: string, dest: string): Promise<void> {
    ...
    try {
      await this.secafsFs.rename(srcAgent, destAgent);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      if (msg.includes("ENOENT") || msg.includes("not found")) {
  ...
  getAllPaths(): string[] {
    // SecAFS doesn't provide a way to list all paths efficiently
    return [];
  }
  ...
  async chmod(path: string, mode: number): Promise<void> {
    ...
    // SecAFS doesn't support chmod yet - this is a no-op
    void mode;
  }
  ...
  async link(existingPath: string, newPath: string): Promise<void> {
    ...
    await this.secafsFs.copyFile(existingAgent, newAgent);
  }
...
/**
 * Create a just-bash compatible filesystem backed by any SecAFS FileSystem.
 *
 * @example Node.js with PostgreSQL:
 * ```ts
 * import { secafs } from "secafs-sdk/just-bash";
 *
 * const fs = await secafs({ postgresUrl: "postgres://localhost/secafs" });
 ...
 *
 * // In a Durable Object:
 * const secaFs = SecAFS.create(ctx.storage);
 * const fs = secafs({ fs: secaFs });
 * const bashTool = createBashTool({ fs });
 * ```
 */
export async function secafs(
  handleOrOptions: SecAFSHandle | FileSystem | import("../../index_node.js").SecAFSOptions,
  mountPoint?: string
): Promise<IFileSystem> {
  ...
  if (typeof (handleOrOptions as FileSystem).stat === 'function' && !('fs' in handleOrOptions)) {
    return new SecAFSWrapper({ fs: { fs: handleOrOptions as FileSystem }, mountPoint });
  }
  // Check if it's an SecAFSHandle (has fs property containing a FileSystem)
  if ("fs" in handleOrOptions && typeof (handleOrOptions as SecAFSHandle).fs?.stat === 'function') {
    return new SecAFSWrapper({ fs: handleOrOptions as SecAFSHandle, mountPoint });
  }
  // Otherwise it's SecAFSOptions for creating a new PostgreSQL-backed SecAFS
  const handle = await SecAFS.open(handleOrOptions as import("../../index_node.js").SecAFSOptions);
  return new SecAFSWrapper({ fs: handle, mountPoint });
}
