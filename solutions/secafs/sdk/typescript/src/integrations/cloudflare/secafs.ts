/**
 * SecAFS - FileSystem implementation using Cloudflare Durable Objects SQLite
 *
 * This implementation uses Cloudflare's Durable Objects SQLite storage API,
 * allowing SecAFS to run on Cloudflare's edge platform.
 *
 ...
 *
 * SecAFS implements the FileSystem interface using Cloudflare's
...
export class SecAFS implements FileSystem {
  ...
  /**
   * Create a SecAFS from a Durable Object storage context.
   *
   * @param storage - The ctx.storage from a Durable Object
   */
  static create(storage: CloudflareStorage): SecAFS {
    const fs = new SecAFS(storage);
    fs.initialize();
    return fs;
  }
  ...
  private ensureRoot(): number {
    ...
    // Set schema version (keep in sync with SECAFS_SCHEMA_VERSION in sdk/rust/src/schema.rs)
    this.storage.sql.exec(
  ...
  async open(path: string): Promise<FileHandle> {
    ...
    return new SecAFSFile(this.storage, ino, this.chunkSize);
  }
