import { SecAFS } from '../../src';

async function main() {
  // Create an agent with persistent storage
  const secafs_inst = await SecAFS.open({ id: 'toolcalls-demo' });
  ...
  const searchId = await secafs_inst.tools.record(
    'web_search',
    startTime1,
    endTime1,
  ...
  const apiId = await secafs_inst.tools.record(
    'api_call',
    startTime2,
    endTime2,
  ...
  const searches = await secafs_inst.tools.getByName('web_search');
  ...
  const recent = await secafs_inst.tools.getRecent(oneMinuteAgo);
  ...
  const stats = await secafs_inst.tools.getStats();
