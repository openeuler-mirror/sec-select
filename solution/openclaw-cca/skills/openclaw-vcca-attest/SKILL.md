---
name: openclaw-vcca-attest
description: Use when verifying vCCA (virtual CCA) Realm integrity, inspecting current REM/RIM measurement values, or diagnosing RBS policy mismatches in an OpenClaw vCCA deployment. Triggers include "查看vcca度量值", "check vCCA REM3", "验证 vRealm 完整性", "vcca attestation failed", or before generating a new policy after component updates.
---

# openclaw-vcca-attest

## Overview

对 vCCA Realm 执行完整远程证明流程，解析并展示当前 REM/RIM 度量值。与 CCA 流程的区别：无需加载内核模块，token 字段路径为 `virt_cca.realm_token`。
...
运行 `openclaw-vcca-attest/scripts/attest.sh`，或按以下步骤手动执行：
...
从 JWT payload 的 `virt_cca.realm_token` 字段提取度量值：
...
| `virt_cca.realm_token 未找到` | token 结构不符 | 确认 RBS 支持 vCCA 证明 |
